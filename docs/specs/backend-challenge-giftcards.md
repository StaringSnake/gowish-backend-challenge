...
## Goal
Complete the highest-priority README backend challenge work within a three-hour timebox: make giftcard listing bounded, prevent references to unknown stores, and support partial redemption with an auditable spend log and remaining balance.
...
- Summary -> counts expired/active cards globally and by store; `totalAmountCents` is remaining balance; only stores with giftcards appear; empty data yields an empty `byStore` object (source: user).
...
- Summary reporting and a full versioned migration system are deferred because of the three-hour timebox.
...
SPEC STATUS: approved; time-constrained scope
